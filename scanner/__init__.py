...
prefab_files = []
dungeon_skip_template = ["BACKDROP", "MASTER"]

console = Console()


def main():
    albion_dir = Config.get("ao-dir") + r"Albion-Online_Data\StreamingAssets\GameData\templates\GREEN"
    second_folder_archiv = Config.get("ao-dir") + r"Albion-Online_Data\StreamingAssets\GameData\templates\GREEN\NewFolder"
    ...
    for file in files:
        try:
            ...
            shutil.move(
                os.path.join(albion_dir, new_file),
                os.path.join(second_folder_archiv, new_file),
            )
        except IOError as e:
...
def check_chest():
    ...
    for template in prefab_files:
        ...
        if not any(
            skip_template in template for skip_template in dungeon_skip_template
        ):
            if "EXIT" in template:
                flag7 = True
